...
function draw() {
  ...
  let trans = [0,0,0]
  if (sensorValue.alpha !== null) {
    trans = getVector(sensorValue.beta, sensorValue.gamma, sensorValue.alpha);
    matTrans = m4.translation(trans[0], trans[1], trans[2])
    let dbg = document.getElementById("debug")
    dbg.innerHTML = matTrans
  }
  let matAccum1 = m4.multiply(translateToPointZero, matAccum);
  let matAccum10 = m4.multiply(m4.multiply(translateToLeft, translateToPointZero), m4.multiply(matTrans, matAccum));
  ...
  if (sensorValue.alpha !== null) {
    gl.uniformMatrix4fv(shProgram.iModelViewMatrix, false, matAccum10);
    if(panner){
      panner.setPosition(trans[0], trans[1], trans[2])
    }
...
function initGL() {
  ...
  sphere.NormalBufferData(CreateSphereSurface())
...
let context, audio, panner, filter;
